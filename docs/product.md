...
| **Antigravity CLI** | Supported | Active hook integration via the shared `~/.gemini/config/hooks.json` (dedicated `"open-island"` named group; verified against agy 1.1.26). Payload is its own camelCase protojson — no `cwd` and no event name in the payload, so the event travels via `--event` and the workspace is recovered from the parent `agy` process. `PreToolUse` entries must print nothing to stdout (any JSON without a `decision` is treated as a deny). Passive discovery from `~/.gemini/antigravity-cli` (conversation DB mtimes, prompt history, summaries DB) continues to run alongside and requires no configuration |
